...
import { CheckCircle, Clock, Wifi, MapPin, Award, Users, Presentation, BookOpen, Clipboard, Filter } from "lucide-react";
...
const ProgramCard = ({ program }) => {
  const getActivityIcon = () => {
    switch(program.type) {
      ...
      case 'Presentation':
        return <Presentation className="w-4 h-4 sm:w-5 sm:h-5 text-purple-600" />;
      case 'Training':
      ...
      default:
        return program.isOnline ? <Wifi className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600" /> : <MapPin className="w-4 h-4 sm:w-5 sm:h-5 text-red-600" />;
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg sm:rounded-xl shadow-sm sm:shadow-md overflow-hidden transition-transform hover:scale-[1.02] flex flex-col h-full border border-gray-200">
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-grow">
        ...
        <h2 className="text-base sm:text-lg font-bold line-clamp-2">{program.title}</h2>
        ...
        <Link href={program.route} className="mt-auto">
          <button className={`w-full py-1.5 sm:py-2 rounded-md text-sm sm:text-base font-medium transition ${
            program.isOnline 
...
const ProgramOverview = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-gray-50">
      ...
      <div className={`flex-1 transition-all duration-300 ${sidebarExpanded ? 'lg:ml-70' : 'lg:ml-30'} pt-4 sm:pt-6 pb-6 px-4 sm:px-6`}>
        ...
        >
          ...
          {(filter.delivery !== "all" || filter.type !== "all") && (
            <span className="ml-auto px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full text-xs">
              {[filter.delivery !== "all", filter.type !== "all"].filter(Boolean).length}
            </span>
          )}
        </button>

        {/* Filter Section */}
        <div className={`${mobileFiltersOpen ? 'block' : 'hidden'} lg:block mb-6 bg-white sm:bg-gray-50 p-3 sm:p-4 rounded-lg shadow-sm sm:shadow-none border border-gray-200 sm:border-none`}>
          <div className="flex items-center gap-2 mb-3 text-gray-700">
            <Filter className="w-4 h-4 sm:w-5 sm:h-5" />
            <span className="font-medium text-sm sm:text-base">Filter Programs</span>
            {mobileFiltersOpen && (
              <button 
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4">
            ...
            <div className="flex-1 min-w-[200px]">
              <h3 className="text-xs sm:text-sm font-medium text-gray-700 mb-2">Delivery Method</h3>
            ...
            {/* Program Type Filter */}
            <div className="flex-1 min-w-[200px]">
              <h3 className="text-xs sm:text-sm font-medium text-gray-700 mb-2">Program Type</h3>
              <div className="flex flex-wrap gap-2">
                <button 
        ...
        {filteredPrograms.length > 0 ? (
          <div className={`grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 ${sidebarExpanded ? 'lg:grid-cols-3 xl:grid-cols-4' : 'lg:grid-cols-4 xl:grid-cols-5'} gap-4 sm:gap-6`}>
          ...
          <div className="text-center py-8 sm:py-12 bg-white sm:bg-gray-50 rounded-lg shadow-sm sm:shadow-none border border-gray-200 sm:border-none">
            <h3 className="text-base sm:text-lg font-medium text-gray-500">No programs found</h3>
