...
export default function UserProfilePage() {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/v1/users/profile/${userId}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      // Refresh user data
      const userRes = await fetch(`http://localhost:3001/api/v1/users/${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handlePasswordSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:3001/api/v1/users/password/${userId}`, {
        method: 'PUT',
        headers: {
              ...
              <div className="w-full md:w-1/3 flex flex-col items-center">
                <div className="relative w-32 h-32 rounded-full overflow-hidden border-2 border-red-500 mb-4">
                  {previewImage ? (
                    ...
                    <Image 
                      src={`http://localhost:3001/uploads/${user.profile.profile_picture.split("\\").pop()}`}
                      alt="Profile picture"
                      layout="fill"
                      objectFit="cover"
                    />
