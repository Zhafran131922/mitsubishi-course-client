...
const ProgramOverview = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col lg:flex-row min-h-screen">
        <Sidebar 
          className="w-full lg:w-30 bg-gray-800 text-white fixed lg:relative top-0 left-0" 
          onExpand={setSidebarExpanded}
        />
        <div className={`${sidebarExpanded ? "lg:ml-64" : "lg:ml-24"} p-6 w-full flex justify-center items-center transition-all duration-300`}>
          <motion.div
            animate={{ rotate: 360 }}
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen">
      {/* Sidebar with expand functionality */}
      <Sidebar 
        className="w-full lg:w-30 bg-gray-800 text-white fixed lg:relative top-0 left-0 z-50" 
        onExpand={setSidebarExpanded}
      />

      {/* Main Content with dynamic margin based on sidebar state */}
      <motion.div
        className={`${sidebarExpanded ? "lg:ml-64" : "lg:ml-24"} p-6 w-full transition-all duration-300`}
        initial="hidden"
        animate="visible"
        variants={container}
      >
