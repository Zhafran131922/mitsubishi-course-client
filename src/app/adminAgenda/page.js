      ...
      <div className="flex flex-col xl:flex-row gap-4 lg:gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 lg:gap-4">
            <div className="bg-gradient-to-br from-gray-900 to-gray-700 p-3 lg:p-4 rounded-xl shadow-lg border border-gray-800 w-full h-32 lg:h-40 flex items-center justify-center">
              <div className="text-center">
                <p className="text-white text-xs lg:text-sm font-light">Total Appointments</p>
                <p className="text-white text-2xl lg:text-3xl font-bold mt-1 lg:mt-2">128</p>
            ...
            <div className="bg-white p-3 lg:p-4 rounded-xl shadow-lg border border-gray-200 w-full h-32 lg:h-40 flex items-center justify-center">
              <div className="text-center">
                <p className="text-gray-700 text-xs lg:text-sm font-light">Upcoming Today</p>
                <p className="text-black text-2xl lg:text-3xl font-bold mt-1 lg:mt-2">9</p>
        ...
        <div className="w-full xl:w-1/2 bg-white p-4 lg:p-6 rounded-xl shadow-xl border border-gray-200 flex flex-col">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 lg:mb-6 gap-3">
            <h2 className="text-xl lg:text-2xl font-bold text-gray-800">Weekly Agenda</h2>
            ...
            <div className="sm:hidden w-full">
              ...
              >
                <span>{format(weekDays[0], "MMM d")} - {format(weekDays[6], "MMM d")}</span>
                <FiChevronRight className={`transition-transform ${showMobileWeekNav ? 'rotate-90' : ''}`} />
              </button>
            ...
            <table className="w-full min-w-[600px] border-collapse">
              <thead>
                <tr className="bg-gradient-to-r from-gray-900 to-gray-800 text-white">
                  <th className="p-2 lg:p-3 text-left rounded-tl-lg text-xs lg:text-sm">Team Member</th>
                  {weekDays.map((day, index) => (
                    <th key={index} className="p-1 lg:p-2 text-center">
                      <div className="flex flex-col items-center">
                        <span className={`text-xs lg:text-sm font-medium ${index === 0 ? "text-red-300" : "text-gray-300"}`}>
                          {format(day, "EEE").charAt(0)}
                        </span>
                        <span className={`text-sm lg:text-base font-bold ${index === 0 ? "text-white" : "text-white"}`}>
                          {format(day, "d")}
                        </span>
                      </div>
                    </th>
                ...
                {users.map((user, i) => (
                  ...
                  >
                    <td className="p-2 lg:p-3 font-medium text-gray-700 flex items-center text-xs lg:text-sm">
                      <div className={`w-6 h-6 lg:w-8 lg:h-8 rounded-full ${user.color} flex items-center justify-center text-white font-bold mr-2 lg:mr-3 text-xs lg:text-sm`}>
                        {user.avatar}
                      </div>
                      <span className="truncate max-w-[100px] lg:max-w-none">{user.name}</span>
                    </td>
                    {weekDays.map((day, index) => {
                      ...
                      return (
                        <td
                          ...
                          onClick={() => isScheduled && setSelectedSchedule({
                            user: user.name,
                            date: format(day, "EEE, MMM d")
                          })}
                        >
                          <div className={`w-6 h-6 lg:w-8 lg:h-8 mx-auto rounded-full flex items-center justify-center transition-all duration-200 cursor-pointer
                            ${isScheduled ? 
        ...
        {selectedSchedule && (
          ...
          >
            <div className="flex justify-between items-start mb-3 lg:mb-4">
              <h3 className="text-lg lg:text-xl font-bold text-gray-800">Appointment Details</h3>
              <button
            ...
            <div className="space-y-3 lg:space-y-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-800 text-sm lg:text-base">{selectedSchedule.user}</p>
                </div>
              </div>
              ...
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-xs lg:text-sm text-gray-500">Date & Time</p>
                  <p className="font-medium text-gray-800 text-sm lg:text-base">{selectedSchedule.date} at 2:00 PM</p>
                </div>
              </div>
