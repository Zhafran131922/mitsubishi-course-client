...
const AdminProgram = () => {
  ...
  const filteredPrograms = programs.filter(({ title, type, isOnline }) => {
    ...
    const matchesType = 
      filter.type === "all" || 
      type.toLowerCase() === filter.type.toLowerCase();
  ...
  const handleDelete = async (program) => {
    ...
    router.push(`/adminPageProgram?id=${program.id}`); // Sesuaikan URL jika berbeda
  };
  ...
  const handleSubmit = async (formData) => {
    try {
      if (currentProgram) {
        const updatedProgram = await ProgramAPI.update(currentProgram.id, formData);
        setPrograms(programs.map(p => p.id === currentProgram.id ? updatedProgram : p));
      } else {
  ...
  // Misalnya: buka modal untuk edit atau tampilkan detail
  console.log('Program clicked:', program);
  // Contoh lain: set program yang akan diedit
  setCurrentProgram(program);
  setIsModalOpen(true);
};
    ...
    <div className="flex flex-col lg:flex-row min-h-screen">
      ...
      <div className="lg:ml-30 p-6 w-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          <h1 className="text-2xl font-bold text-center md:text-left">Manage Programs</h1>
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {[
                  ...
                  { label: "Online", value: "online", count: onlineCount, icon: <Wifi className="w-4 h-4" /> },
                  { label: "Offline", value: "offline", count: offlineCount, icon: <MapPin className="w-4 h-4" /> },
                ].map(({ label, value, count, icon }) => (
                  <button
                    key={value}
                    onClick={() => setFilter((prev) => ({ ...prev, delivery: value }))}
        ...
        <div className="mb-4 text-sm text-gray-500">
          ...
          {(filter.delivery !== "all" || filter.type !== "all") && (
            <span className="ml-2">
              (Filtered:{filter.delivery !== "all" && ` ${filter.delivery}`}{filter.type !== "all" && ` ${filter.type}`})
            </span>
          )}
